...
import dji.sampleV5.aircraft.remote.RemoteControlConfigStore
import dji.sampleV5.aircraft.remote.RemoteControlService
...
class VirtualStickFragment : DJIFragment() {
    ...
    private fun initBtnClickListener() {
        ...
        binding?.btnRemoteStart?.setOnClickListener {
            context?.applicationContext?.let {
                RemoteControlService.start(it)
                ToastUtils.showToast(getString(R.string.remote_service_started))
            }
        }
        binding?.btnRemoteStop?.setOnClickListener {
            context?.applicationContext?.let {
                RemoteControlService.stop(it)
                ToastUtils.showToast(getString(R.string.remote_service_stopped))
            }
        }
    ...
    private fun showRemoteConfigDialog() {
        ...
        val store = RemoteControlConfigStore(activity.applicationContext)
        val current = store.load()
        KeyValueDialogUtil.showInputDialog(
            activity,
            getString(R.string.remote_config_dialog_title),
            store.toJson(current),
            "",
            false,
        ) { input ->
            ...
            try {
                store.save(store.fromJson(input))
                ToastUtils.showToast(getString(R.string.remote_config_saved))
            } catch (e: Exception) {
                ToastUtils.showToast(getString(R.string.remote_config_parse_error))
            }
        }
    }
}
